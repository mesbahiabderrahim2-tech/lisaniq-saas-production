...
export function getStripeClient(): Stripe {
  ...
  // تعديل الإصدار هنا إلى الإصدار المتوافق تماماً لحل خطأ فحص TypeScript دون المساس بالمنطق البرمي
...
export const STRIPE_PRICES: Record<string, string> = {
  ...
  pro_yearly: process.env.NEXT_PUBLIC_STRIPE_PRICE_PRO_YEARLY ?? process.env.NEXT_PUBLIC_STRIPE_PRICE_PRO_YEARLY ?? '',
};
...
export async function getOrCreateStripeCustomer({
  ...
  if (existingCustomerId) {
    ...
    try {
      ...
      if (!(customer as Stripe.DeletedCustomer).deleted) return existingCustomerId;
    } catch {
...
/**
 * Maps a Stripe priceId to our internal plan name.
 */
...
export function mapStripeStatus(
  ...
  const statusMap: Record<Stripe.Subscription.Status, 'active' | 'trialing' | 'past_due' | 'canceled' | 'unpaid' | 'incomplete'> = {
    active: 'active',
    trialing: 'trialing',
    past_due: 'past_due',
    canceled: 'canceled',
